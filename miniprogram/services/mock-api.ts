...
import { getStreakBonus, calculateCheckInScore } from '../utils/scoring';
import { dateKey, formatDuration, getWeekDays, formatRelativeTime } from '../utils/format';
...
function emptyStats(): User['stats'] {
  return { weeklyScore: 0, weeklyCalories: 0, weeklyMinutes: 0, streakDays: 0, rank: 0, checkInDays: 0 };
}

function createSeedCheckIns(): CheckIn[] {
  const now = new Date();
  const items: Array<[string, CheckInInput, number]> = [
  ...
  return items.map(([userId, input, dayOffset], index) => {
    ...
    const calories = calculateCalories({ ...input, weight: user.weight });
    const scoreBreakdown = calculateCheckInScore(input, calories);
    return {
      ...input,
      id: `checkin-${index + 1}`,
      userId,
...
function refreshStats(state: MockState): void {
  const days = getWeekDays();
  state.users.forEach((user) => {
    const own = state.checkIns.filter((checkIn) => checkIn.userId === user.id && checkIn.status === 'valid');
    const weekly = own.filter((checkIn) => days.some((day) => day.key === dateKey(new Date(checkIn.createdAt))));
    const dates = new Set(weekly.map((checkIn) => dateKey(new Date(checkIn.createdAt))));
    ...
    user.stats = { weeklyScore: score + getStreakBonus(dates.size), weeklyCalories: calories, weeklyMinutes: minutes, streakDays: dates.size, rank: 0, checkInDays: dates.size };
  });
...
function groupFor(state: MockState): Group {
  refreshStats(state);
...
function buildCalendar(state: MockState): CalendarDay[] {
  const current = state.currentUserId;
  const ownDates = new Set(state.checkIns.filter((item) => item.userId === current && item.status === 'valid').map((item) => dateKey(new Date(item.createdAt))));
  return getWeekDays().map((day) => ({ ...day, completed: ownDates.has(day.key) }));
}

function buildActivities(state: MockState): Activity[] {
...
function rankingData(state: MockState, type: RankingType): RankingData {
  refreshStats(state);
  const meta: Record<RankingType, { title: string; unit: string }> = {
  ...
  const values = state.users.map((user) => {
    ...
    let value = 0;
    let detail = '';
    if (type === 'score') { value = user.stats.weeklyScore; detail = `${user.stats.checkInDays} 天有效打卡`; }
    if (type === 'calories') { value = user.stats.weeklyCalories; detail = `${user.stats.weeklyMinutes} 分钟运动`; }
    if (type === 'streak') { value = user.stats.streakDays; detail = `${user.stats.weeklyScore} 积分`; }
    if (type === 'gym') { value = own.filter((item) => item.sport === 'gym').reduce((sum, item) => sum + item.duration, 0); detail = '力量与体能训练'; }
    if (type === 'run') { value = Number(own.filter((item) => item.sport === 'run').reduce((sum, item) => sum + (item.distance || 0), 0).toFixed(1)); detail = '累计跑步距离'; }
    if (type === 'basketball') { value = own.filter((item) => item.sport === 'basketball').reduce((sum, item) => sum + item.duration, 0); detail = '球场活跃时长'; }
    if (type === 'pilates') { value = own.filter((item) => item.sport === 'pilates').reduce((sum, item) => sum + item.duration, 0); detail = '核心与柔韧训练'; }
  ...
  return {
    ...
    unit: meta[type].unit,
    updatedAt: '刚刚更新',
    entries: values.map((item, index): RankingEntry => ({ rank: index + 1, user: item.user, value: item.value, unit: meta[type].unit, trend: index === 0 ? 0 : index % 2 === 0 ? -1 : 1, highlighted: item.user.id === state.currentUserId, detail: item.detail }))
  };
}

export function createMockApi(): FitnessApi {
  return {
    async getSession(): Promise<Session> {
      ...
      return { authenticated: true, user, groupId: 'group-brofit', profileCompleted: Boolean(readStorage<boolean>(STORAGE_KEYS.profileCompleted, false)) };
    },
    ...
    async createCheckIn(input: CheckInInput): Promise<CheckInResult> {
      ...
      const calories = calculateCalories({ ...input, weight: user.weight });
      const scoreBreakdown = calculateCheckInScore(input, calories);
      const checkIn: CheckIn = { ...input, id: `checkin-${Date.now()}`, userId: user.id, createdAt: input.createdAt || new Date().toISOString(), calories, score: scoreBreakdown.total, scoreBreakdown, status: 'valid' };
    ...
    async sendNudge(input: SendNudgeInput): Promise<void> {
      const state = getState();
